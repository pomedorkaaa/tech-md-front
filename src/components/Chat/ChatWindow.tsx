...
export default function ChatWindow({
  ...
  const initial = selectedConversation.companyName ? selectedConversation.companyName[0] : '?';
  ...
  return (
    <div className="flex-1 flex flex-col">
      ...
      <div className="flex items-center justify-between px-6 py-3 border-b border-border bg-surface-paper">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-text-muted">{selectedConversation.unread ? 'Новые сообщения' : 'Открыто'}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4">
        {messages.length === 0 ? (
          <p className="text-sm text-text-muted text-center py-12">Нет сообщений. Напишите первым.</p>
        ) : (
          messages.map(msg => (
            <div
              key={msg.id}
      ...
      <div className="px-6 py-4 border-t border-border bg-surface-paper">
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="p-2.5 rounded-xl gradient-primary text-white hover:opacity-90 transition-opacity disabled:opacity-50"
