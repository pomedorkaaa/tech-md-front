      ...
      <div className="gradient-card rounded-xl p-5 sm:p-6 border border-border space-y-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div><label className="block text-xs font-bold text-text-muted uppercase tracking-wider mb-2">Имя</label>
            <input className="w-full bg-surface-elevated border border-border rounded-lg px-4 py-3 text-sm text-text-primary outline-none focus:border-primary/50" placeholder="Ваше имя"/></div>
          <div><label className="block text-xs font-bold text-text-muted uppercase tracking-wider mb-2">Email</label>
            <input className="w-full bg-surface-elevated border border-border rounded-lg px-4 py-3 text-sm text-text-primary outline-none focus:border-primary/50" placeholder="name@email.md"/></div>
        </div>
        <div><label className="block text-xs font-bold text-text-muted uppercase tracking-wider mb-2">Сообщение</label>
          <textarea rows={4} className="w-full bg-surface-elevated border border-border rounded-lg px-4 py-3 text-sm text-text-primary outline-none focus:border-primary/50 resize-none" placeholder="Опишите вашу проблему..."/></div>
        <button className="flex items-center gap-2 px-6 py-3 rounded-lg gradient-primary text-white font-bold text-sm"><Send size={16}/> Отправить</button>
      </div>

      <div className="text-center text-sm text-text-muted">
        <p className="flex items-center justify-center gap-2"><Mail size={14}/> support@techmoldova.md</p>
      </div>
    </div>
  );
}
