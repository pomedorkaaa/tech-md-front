  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 overflow-x-hidden">
      <div className="mb-6 sm:mb-8">
        <h1 className="text-2xl sm:text-3xl font-bold text-text-primary">Панель управления</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 sm:gap-6">
        ...
        <div className="gradient-card rounded-xl p-4 sm:p-6 border border-border">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-base sm:text-lg font-bold text-text-primary">Активные вакансии</h2>
            <button className="text-xs sm:text-sm text-primary hover:text-primary-light flex items-center gap-1">
              Показать все <ArrowRight size={14} />
            </button>
          </div>
          <div className="space-y-3">
            {activeJobs.map(job => (
              <div key={job.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-3 sm:p-4 rounded-xl bg-surface-elevated hover:bg-surface-overlay transition-colors gap-2">
                <div className="min-w-0">
                  <h3 className="font-medium text-sm sm:text-base text-text-primary truncate">{job.title}</h3>
        ...
        <div className="gradient-card rounded-xl p-4 sm:p-6 border border-border">
          <div className="flex items-center justify-between mb-4 gap-2">
            <h2 className="text-base sm:text-lg font-bold text-text-primary">Решения тестов</h2>
            <button className="text-xs sm:text-sm text-primary hover:text-primary-light flex items-center gap-1 shrink-0 whitespace-nowrap">
              Все результаты <ArrowRight size={14} />
            </button>
          </div>
          <div className="space-y-3">
            {testResults.map(result => (
              <div key={result.id} className="flex items-center justify-between p-3 sm:p-4 rounded-xl bg-surface-elevated hover:bg-surface-overlay transition-colors gap-2">
                <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <h3 className="font-medium text-xs sm:text-sm text-text-primary truncate">{result.candidateName}</h3>
                    <p className="text-[10px] sm:text-xs text-text-muted truncate">{result.candidateRole}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2 sm:gap-3 shrink-0">
                  <div className="text-right">
                    ...
                    <p className="text-[10px] sm:text-xs text-text-muted">{result.passedTests}/{result.totalTests} тестов</p>
                  </div>
                  <button className="p-1.5 sm:p-2 rounded-lg hover:bg-surface-overlay transition-colors text-text-muted hover:text-primary">
                    <Eye size={14} />
                  </button>
                </div>
              </div>
      ...
      {/* CTA */}
      <div className="mt-6 sm:mt-8 gradient-card rounded-xl p-4 sm:p-6 border border-primary/20">
        <h3 className="text-base sm:text-lg font-bold text-text-primary mb-2">Начните поиск талантов прямо сейчас</h3>
        ...
        <button className="px-5 sm:px-6 py-3 rounded-xl gradient-primary text-white font-medium hover:opacity-90 transition-opacity text-sm">
          Создать вакансию
        </button>
      </div>
    </div>
